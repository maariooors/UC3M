...
cos:
    ...
    li t2 4   # Aprox para cumplir con un error menor de 0.001 (aprx)
    # li t3 0 # Valor que vamos a devolver
    li t4 0   # Contador del bucle cos (i) (n)
    li t5 -1  # Numero -1
    li t6 0   # Contador del bucle dividendo
    li s1 2   # Numero 2
    li s2 0   # Guardamos (2n)
    li s3 1   # Nos guardamos x^2n
    li s4 0   # Nos guardamos (-1)^n
    li s5 0   # Nos guardamos el dividendo = (-1)^n * x^2n
    li s6 0   # Calculamos el divisor = (2n)

    fcvt.s.w fs3 zero  # Almacenará toda la suma del cos
    mv s7 a0           # Me guardo el valor de a0 para no perderlo

    bucle_sen: bgt t4 t2 end_sen

        mul s2 s1 t4    # Me guardo el 2n
        bucle_dividendo: bge t6 s2 end_dividendo  # Bucle para x^2n
        
            mul s3 s3 s7
            addi t6 t6 1 # Incrementamos el contador que tiene que llegar a 2n
        ...
        end_dividendo:
            ...
            beq s4 zero positivo
                li s4 -1   # Si la n es negativa (-1)^n es -1
            positivo:
                li s4 1    # Si la n es negativa (-1)^n es 1  

            mul s5 s3 s4   # (-1)^n * x^2n

    # Calculamos el (2n)!
    # Usaré el mismo registro s2 ya que tiene el valor de 2n
        ...
        fdiv.s fs2 fs1 fs0  # Dividendo / divisor = ((-1)^n * x^2n) / (2n)!
        fadd.s fs3 fs3 fs2  # Incrementamos la suma

        addi t4 t4 1        # Incremento el contador (n)
        li s3 1             # Reinicio el valor de x^2n
        j bucle_sen
    end_sen:
        fmv.x.w a0 fs3  # Movemos el resultado a a0
        jr ra           # Vamos a la instruccion posterior a la llamada a "cos"

seno:
    # a0 es el parametro "x" que se le pasa a la función
    li t2 4   # Aprox para cumplir con un error menor de 0.001 (aprx)
    # li t3 0 # Valor que vamos a devolver
    li t4 0   # Contador del bucle sen (i) (n)
    li t5 -1  # Numero -1
    li t6 0   # Contador del bucle dividendo
    li s1 2   # Numero 2
    ...
    fcvt.s.w fs3 zero  # Almacenará toda la suma del seno
    mv s7 a0           # Me guardo el valor de a0 para no perderlo
